...
import '../../../data/models/vus.dart';

class FilterStudentRatingNotifier extends ChangeNotifier {
  String city = '';
  String age_gte = '0';
  String age_lte= '50';
  String vus = '';
  ...
  FilterStudentRatingNotifier(){
   ...
   void _setup() async{
    ...
    final city =  await api.getAllCities();
    for(int i = 0; i < city.cities.length; i++){
      cities.add(city.cities[i]);
    }
  ...
  void dropFilter(BuildContext context) async{
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (context) => MainStudent(selectIndex: 1)));
  }
