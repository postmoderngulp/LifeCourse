...
import 'package:hac/layers/presentation/pages/student/about_event.dart';
import 'package:mask_text_input_formatter/mask_text_input_formatter.dart';
import 'package:provider/provider.dart';
import 'package:hac/layers/presentation/notifiers/student/events_students_notifier.dart';
import 'package:hac/layers/presentation/notifiers/student/filter_events_student_notifier.dart';
...
class MasterClass extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final notifier = context.watch<EventsStudentNotifier>();
...
class MasterCLassItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (context) =>  AboutEvent(myEvent: notifier.myEvents[index],))),
      child: Container(
        width: 335.w,
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  SizedBox(
                    height: 30.h,
                    child: ElevatedButton(onPressed: () => notifier.myEvents[index].requested ? null : notifier.sendApplication(notifier.myEvents[index].id), style: ButtonStyle(
                      backgroundColor:  MaterialStatePropertyAll(notifier.myEvents[index].requested ? Colors.white :MyColors.primary700),
                      shape: MaterialStatePropertyAll(RoundedRectangleBorder(borderRadius: BorderRadius.circular(5),side: BorderSide(color: notifier.myEvents[index].requested ? MyColors.primary700 : Colors.transparent)),)
                    ),child: Text(   notifier.myEvents[index].requested  ? "Заявка принята": 'Отправить',style:notifier.myEvents[index].requested  ? FontStylization.addToMcActive : FontStylization.addToMc,)),
                  )
                ],
              ),
              SizedBox(height: 13.h,),
    ...
    return Center(
      child: SizedBox(
        ...
        child: CupertinoTextField(
           ...
           suffix: GestureDetector(
            onTap: () => showModalBottomSheet<dynamic>(
              ...
              builder: (BuildContext context) {
                return  ChangeNotifierProvider(create: ( context) => FilterStudentEventNotifier(),
                child: const Banner());
              },
            ),
            child: Padding(
          ...
          onChanged: (value) {notifier.searchProfile(value, notifier.events);},
...
class DateField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final notifier = context.watch<FilterStudentEventNotifier>();
    return Center(
      child: SizedBox(
        width: 320.w,
        height: 50.h,
